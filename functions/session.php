...
if (isset($_COOKIE['token'])) {
  ...
  if (mysqli_num_rows($result) > 0) {
      session_start();
      $userdbd = $conn->query("SELECT * FROM users WHERE usertoken='$session_id'")->fetch_array();
  ...
  else
  {
      ...
      echo '<script>window.location.replace("/login?r='.$fullUrl.'");</script>';
      exit();
  }
}
else
{
  ...
  header('location: /login?r='.$fullUrl);
  exit();
}
?>
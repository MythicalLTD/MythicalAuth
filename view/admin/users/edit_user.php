...
if (isset($_GET['edit_user'])) {
    if (!$_GET['id'] == "") {
        ...
        if (mysqli_num_rows($result) > 0) {
            $user_info = $conn->query("SELECT * FROM users WHERE id = '" . $_GET['id'] . "'")->fetch_array();
            ...
            if (!$username == "" || $firstName == "" || $lastName == "" || $email == "" || $avatar == "" || $role == "") {
                if (!decrypt($user_info['username'], $ekey) == $username || !$email == $user_info['email']) {
                    ...
                    if ($role == "admin") {
                        $conn->query("UPDATE `users` SET `role` = 'administrator' WHERE `users`.`id` = " . $_GET['id'] . ";");
                    } else if ($role == "user") {
                                        ...
                                        <div class="row">
                                            <div class="mb-3 col-md-6">
                                                ...
                                                <input class="form-control" type="text" id="banned" name="banned"
                                                    ...
                                                    placeholder="This user is not banned" readonly="true" disabled="true"/>
                                            </div>
                                            <div class="mb-3 col-md-6">
                                            ...
                                            <div class="mb-3 col-md-6">
                                                ...
                                                <input class="form-control" type="text" id="registred" name="registred"
                                                    value="<?= $user_info['registred'] ?>"
                                                    placeholder="<?= $user_info['registred'] ?>" readonly="true" disabled="true"/>
                                            </div>
                                            <div class="mb-3 col-md-6">
                                                <label for="verification_code" class="form-label">Verification Code</label>
                                                <input class="form-control" type="text" id="verification_code" name="verification_code"
                                                    value="<?= $user_info['verification_code'] ?>"
                                                    placeholder="User is verified" readonly="true" disabled="true"/>
                                            </div>
                                        </div>
                                    </div>
                                </div>
